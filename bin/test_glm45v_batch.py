...
def load_model(model_path: str):
    print("Loading GLM-4.5V model and processor...")
...
def run_evaluation(question: str, image_path: str, model, processor):
    """
    Run evaluation using GLM-4.5V model.
    """
    if not image_path or not os.path.exists(image_path):
        return f"[ERROR] Image not found: {image_path}"
    
    try:
        # Prepare messages in GLM-4.5V format following official documentation
        messages = [
            {
                "role": "user",
                "content": [
        ...
        # Remove token_type_ids if present (as shown in reference code)
...
def run_dataset(
    ...
    for idx, sample in enumerate(samples):
        ...
        def is_valid_glm45v_response(response: str) -> bool:
            """Check if GLM-4.5V response is valid"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if is_valid_glm45v_response(assistant_response):
...
def parse_args():
    parser = argparse.ArgumentParser(description="Batch inference for GLM-4.5V on MEDVQA datasets")
    ...
    parser.add_argument("--random_seed", type=int, default=42, help="Random seed for reproducible shuffling")
    parser.add_argument("--model_path", type=str, default="zai-org/GLM-4.5V", help="GLM-4.5V model path (HF or local)")
    parser.add_argument(
        "--images_root",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/home/xc1490/xc1490/projects/medvqa_2025/output/glm45v",
