...
# Enforce offline-friendly behavior on HPC to avoid network calls
os.environ.setdefault("HF_HUB_OFFLINE", "1")
os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")
# Use a shared cache on scratch to maximize reuse across jobs/nodes (adjust if needed)
os.environ.setdefault("HF_HOME", "/scratch/xc1490/hf_cache")


def load_model(model_path: str):
    print("Loading Qwen2.5-VL-32B-Instruct model and processor...")
    try:
        ...
        raise RuntimeError(
            "Failed to load Qwen2.5-VL-32B offline. Ensure a complete local snapshot "
            "exists for both model and processor (config, shards, tokenizer/processor)."
        ) from e
...
def run_evaluation(question: str, image_path: str, model, processor):
    """
    Run evaluation using Qwen2.5-VL-32B-Instruct model.
    """
...
def run_dataset(
    ...
    for idx, sample in enumerate(samples):
        ...
        if key in processed_keys:
            continue
        def is_valid_qwen_response(response: str) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if is_valid_qwen_response(assistant_response):
...
def parse_args():
    parser = argparse.ArgumentParser(description="Batch inference for Qwen2.5-VL-32B-Instruct on MEDVQA datasets")
    parser.add_argument("--datasets", nargs="*", default=["pvqa", "slake", "rad"], choices=["pvqa", "slake", "rad"], help="Datasets to run")
    parser.add_argument("--random_dataset", action="store_true", default=True, help="Randomly select one dataset from the specified datasets")
    parser.add_argument("--random_seed", type=int, default=None, help="Random seed for reproducible shuffling")
    parser.add_argument("--model_path", type=str, default="Qwen/Qwen2.5-VL-32B-Instruct", help="Qwen2.5-VL-32B-Instruct model path (HF or local)")
    parser.add_argument(
        "--images_root",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/home/xc1490/xc1490/projects/medvqa_2025/output/qwen25vl32b",
